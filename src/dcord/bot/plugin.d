...
/**
  PluginState is a class that encapsulates all run-time states required for a
  plugin to exist. It's purpose is to allow for hot-reloading and replacing
  of plugin code, without having to destroy or rebuild run-time data.
*/
class PluginState {
  ...
  this(Plugin plugin, PluginOptions opts) {
    ...
    if (this.options.useStorage) this.storage = new Storage(plugin.storagePath);
    if (this.options.useConfig) this.config = new Storage(plugin.configPath);
  }
}
